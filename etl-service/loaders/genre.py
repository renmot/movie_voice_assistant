...
from data_structures.indices import genres_index
from data_structures.tables import ElasticSearchGenre
...
class PostgresExtractor:
    ...
    def extract_genres(self, select_query: str, params: tuple) -> list[dict]:
...
class ElasticSearchLoader:
    ...
    def check_or_create_index(self):
        ...
        if not self.client.indices.exists(index="genres"):
            self.client.indices.create(**genres_index)
            logging.info('Created index "genres" in ElasticSearch DB')
    ...
    @staticmethod
    def gendata(data: list[ElasticSearchGenre]):
        """Generates structure of the index documents."""
        for row in data:
            yield {"_index": "genres", "_id": row.id, "_source": row.json()}
