...
from data_structures.indices import persons_index
from data_structures.tables import ElasticSearchPerson
...
class PostgresExtractor:
    ...
    def extract_persons(self, select_query: str, params: tuple) -> list[dict]:
...
class ElasticSearchLoader:
    ...
    def check_or_create_index(self):
        ...
        if not self.client.indices.exists(index="persons"):
            self.client.indices.create(**persons_index)
            logging.info('Created index "persons" in ElasticSearch DB')
    ...
    @staticmethod
    def gendata(data: list[ElasticSearchPerson]):
        """Generates structure of the index documents."""
        for row in data:
            yield {"_index": "persons", "_id": row.id, "_source": row.json()}
